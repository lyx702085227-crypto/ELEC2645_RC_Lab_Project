...
/**
 * @brief Finds the nearest standard E24 resistor/capacitor value.
 * * Decomposes the input value into mantissa and exponent (logarithmic scale),
 * compares the mantissa against the standard E24 series array, and 
 * reconstructs the closest standard component value.
 * * @param val The calculated ideal resistance or capacitance.
 * @return float The closest standard E24 value.
 */
...
int levenshtein_distance(const char *s1, const char *s2) {
    ...
    int d[50][50]; 
    // [Algorithm] Initialize DP Matrix (Dynamic Programming)
    // d[i][j] will hold the distance between the first i chars of s1 and first j chars of s2
    for (int i = 0; i <= len1; i++) d[i][0] = i;
    for (int j = 0; j <= len2; j++) d[0][j] = j;
    
    // [Algorithm] Compute distances
    for (int i = 1; i <= len1; i++) {
        for (int j = 1; j <= len2; j++) {
            // Cost is 0 if characters match, 1 if substitution is needed
            int cost = (s1[i - 1] == s2[j - 1]) ? 0 : 1;
            
            // Calculate minimum operation: Deletion, Insertion, or Substitution
            int del = d[i - 1][j] + 1;
            int ins = d[i][j - 1] + 1;
...
/**
 * @brief Parses user command with Fuzzy Logic (Levenshtein Distance).
 * * This function calculates the edit distance between the user's input
 * and known commands to handle typos (e.g., correcting "settup" to "setup").
 * * @param input The raw command string entered by the user.
 * @return int The ID of the matched command, or 0 if no match found.
 */
...
/**
 * @brief Renders the simulation results as an ASCII waveform.
 * * Uses a Coordinate Mapping Algorithm to normalize floating-point voltage 
 * values onto a discrete character grid (Terminal UI).
 * Handles the quantization of continuous signals into discrete text rows.
 * * @param params Pointer to the circuit parameters and simulation data.
 */
void plot_results(const CircuitParams *params) {
    ...
    for (int r = height; r >= 0; r--) {
        // Normalize current row to Voltage domain
...
// [AI fitting function]
/**
 * @brief AI Module: Simulates noisy signals and performs parameter recovery.
 * * 1. Signal Injection: Adds Random Thermal Noise & 50Hz Mains Hum to the ideal signal.
 * 2. Visualization: Uses a "Multi-Pass" algorithm to simulate oscilloscope phosphor persistence.
 * 3. Regression: Applies Least Squares method to estimate the true Time Constant (Tau)
 * from the chaotic data cloud.
 * * @param params The ideal circuit parameters to serve as the ground truth.
 */
void ai_noisy_data_fitting(const CircuitParams *params) {
    ...
    for (int pass = 0; pass < NUM_PASSES; pass++) {
        for (int c = 0; c < CANVAS_COLS; c++) {
            ...
            // 1. Random Thermal Noise
            float noise_random = params->Vs * ((rand() % 100 - 50) / 1000.0f); 
            
            #ifndef M_PI
            #define M_PI 3.14159265358979323846
            #endif
            // 2. 50Hz Mains Hum (Simulated Ripple)
            // Using sinf() to simulate AC interference
            ...
            // Superimpose noise onto ideal signal
            ...
            // [AI Core] Data Collection for Linear Regression
            // Only use valid data points to recover the trend
            if (c > 0 && v_display > 0.001f && v_display < params->Vs) {
                // Linearize the exponential equation: ln(1 - V/Vs) = -t/tau
    ...
    // [Math] Least Squares Calculation
    // Slope m = sum(t*y) / sum(t^2), and Tau = -1/slope
